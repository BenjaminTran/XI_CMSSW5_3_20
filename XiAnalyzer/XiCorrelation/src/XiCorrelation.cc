...

{
    ...
    /* _trkSrc( consumes<reco::TrackCollection>(
     * iConfig.getParameter<edm::InputTag>( "trkSrc" ) ) );
     */
    xiCollection_   = iConfig.getParameter<edm::InputTag>("xiCollection");
    //_xiCollection( consumes<reco::VertexCompositeCandidateCollection>(
    //iConfig.getParameter<edm::InputTag>( "xiCollection" ) ) );
    ...
    xiMassHigh_     = iConfig.getParameter<double>( "xiMassHigh" );
    xiMassLow_      = iConfig.getParameter<double>( "xiMassLow" );
...
XiCorrelation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if( EtaPtCutnTracks >= multLow_ && EtaPtCutnTracks < multHigh_ ){
        ...
        for(reco::VertexCompositeCandidateCollection::const_iterator xiCand =
            ...
            //
            //if( xi_pT < ptMin_xi_ || xi_pT > ptMax_xi_ && dopTcut_ ) continue;
        ...
        int skipCounter = 0; // Used to determine how many Xi candidates are omitted due to peak and sideband mass cuts
        XiPerEvt->Fill( pepVect_Xi_size ); //For normalization of correlation plot but don't use this for peak and sideband regions use the ones defined below in the appropriate spots
        TrkassPerEvt->Fill( pepVect_trkass_size );

        /*
        for( int i = 0; i < ( int )xiMass->size(  ); i++ )
        {
            if( (*xiMass)[i] < xiMassLow_ ) 
                skipCounter++;
            if( (*xiMass)[i] > xiMassHigh_  ) skipCounter++;
        }
        */
        ...
        {
            if( (*xiMass)[xi_trg] > xiMassLow_  ){
                cout << "Lower cut satisfied" << endl;
                cout <<  (*xiMass)[xi_trg]  << endl;
                if( (*xiMass)[xi_trg] < xiMassHigh_ ){
                    cout << "Upper cut satisfied" << endl;
                    ...
                    for( int assoc = 0; assoc < pepVect_trkass_size; assoc++ )
                    {
                        ...
                        //SignalXiPeak->Fill( dEta, dPhi, 1.0/(pepVect_Xi_size - skipCounter) );
                        SignalXiPeak->Fill( dEta, dPhi, 1.0/pepVect_Xi_size );
                    }
               }
            }
        }
        XiPerEvtPeak->Fill( pepVect_Xi_size - skipCounter );
        ...
        for( int i = 0; i < ( int )xiMass->size(  ); i++ )
        {
            if( (*xiMass)[i] > xiMassLow_ && (*xiMass)[i] < xiMassHigh_ ) 
                skipCounter++;
        }
        */
        ...
        for( int xi_trg = 0; xi_trg < pepVect_Xi_size; xi_trg++ )
        {
            if( (*xiMass)[xi_trg] > xiMassLow_ && (*xiMass)[xi_trg] < xiMassHigh_ ) continue;
                ...
                for( int assoc = 0; assoc < pepVect_trkass_size; assoc++ )
                {
                    ...
                    // To reduce jet fragmentation contributions
                    if( fabs(dEta) < 0.028 && fabs( dPhi ) < 0.02  ) continue;
                    //SignalXiSide->Fill( dEta, dPhi, 1.0/(pepVect_Xi_size - skipCounter) );
                    SignalXiSide->Fill( dEta, dPhi, 1.0/pepVect_Xi_size );
                }
        }
        XiPerEvtSide->Fill( pepVect_Xi_size - skipCounter );
...
XiCorrelation::beginJob()
{
    ...
    XiPerEvtPeak           = fs->make<TH1D>( "XiPerEventPeak", "#Xi per Event ( Peak )", 1500, 0, 1500 );
    XiPerEvtSide           = fs->make<TH1D>( "XiPerEventSide", "#Xi per Event ( Sideband )", 1500, 0, 1500 );
    ...
    BackgroundXiPeak   = fs->make<TH2D>( "BackgroundPeak", "Bkg Peak; #Delta#eta;#Delta#phi ", 33, -4.95, 4.95, 31, -( 0.5 - 1.0/32 )*PI, ( 1.5 - 1.0/32 )*PI );
    BackgroundXiSide   = fs->make<TH2D>( "BackgroundSide", "Bkg Side; #Delta#eta;#Delta#phi ", 33, -4.95, 4.95, 31, -( 0.5 - 1.0/32 )*PI, ( 1.5 - 1.0/32 )*PI );
    SignalXi           = fs->make<TH2D>( "Signal", "Sig #Delta#eta;#Delta#phi", 33, -4.95, 4.95, 31, -( 0.5 - 1.0/32 )*PI, ( 1.5 - 1.0/32 )*PI );
    SignalXiPeak       = fs->make<TH2D>( "SignalPeak", "Sig Peak; #Delta#eta;#Delta#phi ", 33, -4.95, 4.95, 31, -( 0.5 - 1.0/32 )*PI, ( 1.5 - 1.0/32 )*PI );
    SignalXiSide       = fs->make<TH2D>( "SignalSide", "Sig Side; #Delta#eta;#Delta#phi ", 33, -4.95, 4.95, 31, -( 0.5 - 1.0/32 )*PI, ( 1.5 - 1.0/32 )*PI );
...
XiCorrelation::endJob() 
{
    ...
    for( int bkgnum = 0; bkgnum<bkgnum_; bkgnum++ )
    {
        ...
        for( int nevt_ass=0; nevt_ass<PepVect2_ass_size; nevt_ass++ )
        {
            ...
            /*
            for( int i = 0; i < ( int )xiMassTmp.size(  ); i++ )
            {
                if( xiMassTmp[i] < xiMassLow_ ) 
                    skipCounter++;
                if( xiMassTmp[i] > xiMassHigh_  ) skipCounter++;
            }
            */

            for( int ntrg=0; ntrg<nMult_trg; ntrg++ )
            {
                if( xiMassTmp[ntrg] > xiMassLow_ && xiMassTmp[ntrg] < xiMassHigh_ ){
                    ...
                    for( int nass=0; nass<nMult_ass; nass++ )
                    {
                        ...
                        //BackgroundXiPeak->Fill( dEta, dPhi, 1.0/(nMult_trg - skipCounter) );
                        BackgroundXiPeak->Fill( dEta, dPhi, 1.0/nMult_trg );
    ...
    for( int bkgnum = 0; bkgnum<bkgnum_; bkgnum++ )
    {
        ...
        for( int nevt_ass=0; nevt_ass<PepVect2_ass_size; nevt_ass++ )
        {
            ...
            for( int i = 0; i < ( int )xiMassTmp.size(  ); i++ )
            {
                if( xiMassTmp[i] > xiMassLow_ && xiMassTmp[i] < xiMassHigh_ ) skipCounter++;
            }
            */

            for( int ntrg=0; ntrg<nMult_trg; ntrg++ )
            {
                if( xiMassTmp[ntrg] > xiMassLow_ && xiMassTmp[ntrg] < xiMassHigh_ ) continue;
                    TVector3 pvectorTmp_trg = pepVectTmp_trg[ntrg];
                    double eta_trg = pvectorTmp_trg.Eta(  );
                    double phi_trg = pvectorTmp_trg.Phi(  );
                    ...
                    for( int nass=0; nass<nMult_ass; nass++ )
                    {
                        ...
                        //BackgroundXiSide->Fill( dEta, dPhi, 1.0/(nMult_trg - skipCounter) );
                        BackgroundXiSide->Fill( dEta, dPhi, 1.0/nMult_trg );
                    }
